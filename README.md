...
If you need to build the interface with the legacy Cython 0.29 toolchain,
run `make interface_legacy`. This installs the pinned Cython into
`python_legacy/` (local to the repo) and builds the extensions with that
environment. A corresponding `make complex_interface_legacy` target is also
available.
