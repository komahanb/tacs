...
PHI[2,:] = phi.subs(x,L/2)
PHI[3,:] = phi.diff(x).subs(x,L/2)
PHI[4,:] = phi.subs(x,L)
PHI[5,:] = phi.diff(x).subs(x,L)
...
print 'disp  shape function', N[4]
print 'slope shape function', N[5]

A = sym.Symbol('A')
rho = sym.Symbol('rho')
M = (N.transpose()*rho*A*N).integrate((x, 0, L))
for i in xrange(6): #ndofoernode*nnodes
    print ("M[%s,:] = ") % (i) , (M[i,:])[:]
